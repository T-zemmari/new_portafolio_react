...
const Hero = () => {
  return (
    <>
    <section className={`w-full h-full flex flex-col md:flex-row md:items-start mt-28 lg:mt-32 px-10 py-0 lg:px-40 lg:py-5 `}>
      <div className={`w-full h-5/6 sm:w-3/6 lg:w-3/6 ${my_styles.paddingX} flex flex-row items-start justify-center gap-5`}>
      ...
      <div className={`w-full sm:w-3/6 lg:w-3/6 h-6/6 ${my_styles.paddingX} md:-mt-0  flex flex-row justify-center items-start`}>
        <div className={`contenedor_imagnen w-[55rem] max-w-96 h-[50rem] md:mt-0 border border-indigo-600 rounded-full bg-[url('${miImagUno}')]  bg-center bg-cover`} style={{backgroundImage:`url(${miImagUno})`}}></div>
      </div>
    </section>
    </>
  );
